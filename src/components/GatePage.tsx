...
const roles: { role: UserRole; label: string; icon: typeof Crown; desc: string }[] = [
  { role: 'guest', label: 'GUEST', icon: Crown, desc: 'Explore & book our exclusive offerings' },
  { role: 'staff', label: 'STAFF', icon: Users, desc: 'Front office operations' },
  { role: 'admin', label: 'ADMIN', icon: Shield, desc: 'Back office management' },
];
...
export default function GatePage() {
  ...
  const handleLogin = () => {
    setError('');
    if (!username.trim() || !pin.trim()) { setError('All fields are required'); return; }
    const ok = login(username, pin, selectedRole);
  ...
  const handleRegister = () => {
    ...
    const ok = register(username, pin);
    if (!ok) setError('Username already taken.');
  };

  const handleBiometric = (role: UserRole) => {
          ...
          <div className="bg-gradient-card border border-border rounded-lg p-6 space-y-4">
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                />
                <button onClick={() => setShowPin(!showPin)} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
